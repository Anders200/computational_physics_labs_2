...
//    • Zero row i and column i in both K (stiffness) and O (overlap).
//    • Set K_{ii} = -2  →  H_{ii} = ½·K_{ii} = -1  (scaled in Python)
//    • Set O_{ii} =  1
//  This decouples boundary nodes and gives spurious eigenvalue -1
//  which is trivially filtered in Python (all physical eigenvalues > 0).
// ============================================================
...
void FEM::solve()
{
    ...
    spdlog::info("CG starting: n={}, ||b||={:.4e}, tol={:.4e}", n, b_norm, tol_abs);
    ...
    for (; iter < cg_max_iter; ++iter) {
        ...
        if (std::sqrt(rr_new) < tol_abs) { iter++; break; }
